...
func reduceGraph(deBruijnGraph map[string]Node) map[string]Node {
	...
	for vertex := range deBruijnGraph {
		if len(deBruijnGraph[vertex].InEdges) != 1 {
			starting_vertices = append(starting_vertices, vertex)
		} else {
			previous_vertex := ""
			for edge := range deBruijnGraph[vertex].InEdges {
				previous_vertex = edge
				break
			}
			if len(deBruijnGraph[previous_vertex].OutEdges) != 1 {
				starting_vertices = append(starting_vertices, vertex)
			}
		}
	}
	for _, vertex := range starting_vertices {
		for {
			if len(deBruijnGraph[vertex].OutEdges) == 1 {
				otherVertex := ""
				for edge := range deBruijnGraph[vertex].OutEdges {
					otherVertex = edge
					break
				}
				if len(deBruijnGraph[otherVertex].InEdges) == 1 {
					if len(deBruijnGraph[otherVertex].OutEdges) == 1 {
						// reduce nodes vertex, otherVertex, furtherVertex to newFirstVertex and newSecondVertex
						furtherVertex := ""
						for edge := range deBruijnGraph[otherVertex].OutEdges {
							furtherVertex = edge
							break
						}
						if len(deBruijnGraph[furtherVertex].InEdges) > 1 {
							break
						}
						...
						newFirstVertexNode := Node{
							kmer:     newFirstVertex,
							OutEdges: make(map[string]int),
							InEdges:  deBruijnGraph[vertex].InEdges,
						}
						...
						newSecondVertexNode := Node{
							...
							OutEdges: deBruijnGraph[furtherVertex].OutEdges,
							InEdges:  make(map[string]int),
						}

						occurances := min(deBruijnGraph[vertex].OutEdges[otherVertex], deBruijnGraph[otherVertex].OutEdges[furtherVertex])
						newFirstVertexNode.OutEdges[newSecondVertex] = occurances
						newSecondVertexNode.InEdges[newFirstVertex] = occurances

						for previousVertex := range deBruijnGraph[vertex].InEdges {
							deBruijnGraph[previousVertex].OutEdges[newFirstVertex] = deBruijnGraph[previousVertex].OutEdges[vertex]
							delete(deBruijnGraph[previousVertex].OutEdges, vertex)
						}
						for nextVertex := range deBruijnGraph[furtherVertex].OutEdges {
							deBruijnGraph[nextVertex].InEdges[newSecondVertex] = deBruijnGraph[nextVertex].InEdges[furtherVertex]
							delete(deBruijnGraph[nextVertex].InEdges, furtherVertex)
						}

						deBruijnGraph[newFirstVertex] = newFirstVertexNode
						deBruijnGraph[newSecondVertex] = newSecondVertexNode
						delete(deBruijnGraph, vertex)
						delete(deBruijnGraph, otherVertex)
						delete(deBruijnGraph, furtherVertex)

						vertex = newFirstVertex

					} else if len(deBruijnGraph[otherVertex].OutEdges) == 0 {
						// reached end of the path
						// reduce vertex, otherVertex to newVertex
						newVertex := vertex[:1] + otherVertex
						newVertexNode := Node{
							kmer:     newVertex,
							OutEdges: deBruijnGraph[otherVertex].OutEdges,
							InEdges:  deBruijnGraph[vertex].InEdges,
						}
						for previousVertex := range deBruijnGraph[vertex].InEdges {
							deBruijnGraph[previousVertex].OutEdges[newVertex] = deBruijnGraph[previousVertex].OutEdges[vertex]
							delete(deBruijnGraph[previousVertex].OutEdges, vertex)
						}
						deBruijnGraph[newVertex] = newVertexNode
						delete(deBruijnGraph, vertex)
						delete(deBruijnGraph, otherVertex)
						break
					} else {
...
func walkReducedGraphHelper(deBruijnGraph map[string]Node, current_vertex string, visited map[string]bool, current_path string, paths *[]string) {
	...
	for next_vertex := range deBruijnGraph[current_vertex].OutEdges {
		if !visited[next_vertex] {
			walkReducedGraphHelper(deBruijnGraph, next_vertex, visited, current_path, paths)
		} else {
			// found a cycle
			*paths = append(*paths, current_path)
		}
...
func walkReducedGraph(deBruijnGraph map[string]Node) []string {
	paths := []string{}
